...
function simulateTask(task) {
  ...
  const actualSpeedup = Math.max(3, baseSpeedup * randomFactor);
  ...
  return {
    ...
    actualMinutes: success ? actualMinutes : task.estimatedBaselineMinutes * 0.8,
    speedup: success ? actualSpeedup : 1.25,
    success,
    timestamp: new Date()
  };
}
 ...
 */
function analyzeByCategory(results, groupBy) {
 ...
 */
function testTenXHypothesis(results) {
 ...
 * Power analysis
 */
function calculatePower(results) {
  const speedups = results.filter(r => r.success).map(r => r.speedup);
  const n = speedups.length;
  const observedEffect = (mean(speedups) - 10) / stdDev(speedups);

  const ncp = observedEffect * Math.sqrt(n);
  const achievedPower = 1 - tCDF(1.645 - ncp, n - 1);
...
function generateMarkdownReport(report) {
  const { overallStats, byCategory, byComplexity, hypothesisTest, powerAnalysis, results } = report;
