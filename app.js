...
const FIRMS_KEY = 'f6249be20191be88f901d1c7305350a0';
...
(function checkWebGL() {
  ...
  console.log('[SIGINT-MAP] WebGL Info:', JSON.stringify(info));
...
try {
  ...
  console.log('[SIGINT-MAP] Earth shader compiled successfully');
} catch(shaderErr) {
...
// Using codetabs CORS proxy first, then direct, then unpkg CDN fallback
const NASA_TEXTURE_URL = 'https://eoimages.gsfc.nasa.gov/images/imagerecords/74000/74393/world.topo.200412.3x5400x2700.jpg';
const NASA_TEXTURE_PROXY = 'https://api.codetabs.com/v1/proxy/?quest=' + encodeURIComponent(NASA_TEXTURE_URL);
const NASA_TEXTURE_FALLBACK = 'https://unpkg.com/three-globe@2.31.0/example/img/earth-blue-marble.jpg';

updateLoading(28, 'FETCHING NASA SATELLITE IMAGERY...');

// Try loading with proxy first, then direct, then unpkg CDN fallback
...
// ---- SUN POSITION FROM REAL UTC TIME ----
// Computes the real sub-solar point from current UTC time
function getSunDirection() {
  const now = new Date();
  const utcHours = now.getUTCHours() + now.getUTCMinutes() / 60 + now.getUTCSeconds() / 3600;
  // Solar declination (approximate — assumes current date March 2)
  const dayOfYear = 61; // March 2 ≈ day 61
  const decl = -23.45 * Math.cos(DEG2RAD * 360 / 365 * (dayOfYear + 10));
  // Hour angle: solar noon at meridian, -180° to 180°
  const hourAngle = (utcHours - 12) * 15; // 15° per hour
  // Sub-solar point lat/lon
  const sunLat = decl;
  const sunLon = -hourAngle; // negative because east is positive in our convention
  return latLonToVec3(sunLat, sunLon, 1).normalize();
}
...
const missileParticles = []; // particle trail emitters
...
async function fetchOpenSky() {
  const urls = [
    ...
    'https://api.codetabs.com/v1/proxy/?quest=https://opensky-network.org/api/states/all?lamin=12&lomin=30&lamax=42&lomax=65'
  ];
...
function setApiStatus(status) {
  if (status === 'live') {
    openskyDot.className = 'status-dot status-dot--live';
    openskyTag.textContent = 'LIVE';
    openskyTag.className = 'source-item__tag';
    aircraftStatusEl.innerHTML = '<span class="status-dot status-dot--live"></span> LIVE';
  } else {
    openskyDot.className = 'status-dot status-dot--offline';
    openskyTag.textContent = 'OFFLINE';
    openskyTag.className = 'source-item__tag source-item__tag--sim';
    aircraftStatusEl.innerHTML = '<span class="status-dot status-dot--offline"></span> OFFLINE';
  }
}

function processAircraftData(states) {
  // Clear old markers
  aircraftMarkers.forEach(m => {
    layers.aircraft.remove(m);
    const idx = pickableObjects.indexOf(m);
    if (idx > -1) pickableObjects.splice(idx, 1);
  });
  ...
  if (liveAircraft.length > 0) {
    aircraftListEl.innerHTML = liveAircraft.slice(0, 6).map(ac =>
      `<div class="aircraft-item">
        <span class="aircraft-item__callsign">${ac.callsign}</span>
        <span class="aircraft-item__alt">${ac.alt}m</span>
      </div>`
    ).join('');
...
events.forEach((evt) => {
  ...
  item.innerHTML = `
    <div class="event-item__dot" style="background: ${evt.color};"></div>
    <div class="event-item__content">
      ...
      <div class="event-item__text">${evt.text}</div>
      <div class="event-item__source">
        <a href="${evt.url}" target="_blank" rel="noopener noreferrer">${evt.source}</a>
      </div>
    </div>
  `;
...
function openDetailPanel(data) {
  ...
  document.getElementById('detail-coords').textContent = data.lat && data.lon ? `${data.lat.toFixed(4)}°N, ${data.lon.toFixed(4)}°E` : 'N/A';
  ...
  document.getElementById('detail-source').innerHTML = data.url
    ? `<a href="${data.url}" target="_blank" rel="noopener noreferrer" style="color:var(--color-air)">${data.source || 'OSINT'}</a>`
    : (data.source || 'OSINT / PUBLIC SOURCE');
...
let frameCount = 0;
const starOpacities = new Float32Array(3000).map(() => 0.3 + Math.random() * 0.7);
...
function animate(now) {
  ...
  impactMarkers.forEach((im, i) => {
    const ring0 = im.rings && im.rings[0];
    const ring1 = im.rings && im.rings[1];
...
console.log('SIGINT-MAP // Operation Epic Fury — 4D Conflict Tracker initialized');
console.log('Sections 16–35 loaded. Data from public reporting. Not for operational use.');
...
async function fetchRSSViaProxy(feedUrl, timeoutMs = 12000) {
  const strategies = [
    // Strategy 1: allorigins raw proxy (CORS *) + client-side XML parsing
    async () => {
    ...
    // Strategy 2: codetabs proxy (CORS) + client-side XML parsing
    async () => {
    ...
    // Strategy 3: rss2json via allorigins wrapper (CORS) 
    async () => {
      const rss2jsonUrl = `https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(feedUrl)}`;
      const url = `https://api.allorigins.win/get?url=${encodeURIComponent(rss2jsonUrl)}`;
      const controller = new AbortController();
      const t = setTimeout(() => controller.abort(), timeoutMs);
      const res = await fetch(url, { signal: controller.signal });
      clearTimeout(t);
      if (!res.ok) throw new Error('allorigins+rss2json ' + res.status);
      const outer = await res.json();
      const inner = JSON.parse(outer.contents);
      if (!inner.items) throw new Error('No items from rss2json');
      return inner.items.map(i => ({ title: i.title || '', link: i.link || '', pubDate: i.pubDate || '' }));
    },
  ];
...
async function refreshNews() {
  try {
    ...
    if (items.length > 0) {
      allNewsItems = items;
      renderNewsFeed();
      if (rssStatusEl) rssStatusEl.innerHTML = '<span class="status-dot status-dot--live"></span> LIVE';
    } else {
      if (rssStatusEl) rssStatusEl.innerHTML = '<span class="status-dot status-dot--offline"></span> NO DATA';
    }
  } catch(e) {
    if (rssStatusEl) rssStatusEl.innerHTML = '<span class="status-dot status-dot--offline"></span> ERROR';
  }
}
...
function renderNewsFeed() {
  ...
  newsFeed.innerHTML = allNewsItems.map(item => {
    ...
    return `<a class="news-item" href="${item.link}" target="_blank" rel="noopener noreferrer">
      <div class="news-item__dot" style="background: ${item.color};"></div>
...
// seismicRings hoisted to top (TDZ fix)
...
async function fetchUSGS() {
  ...
  try {
    ...
    const res = await fetch(url, { signal: controller.signal });
    clearTimeout(timeout);
    if (!res.ok) throw new Error('USGS response not ok');
    const data = await res.json();
...
function setSeismicStatus(status) {
  if (status === 'live') {
    if (seismicDot) seismicDot.className = 'status-dot status-dot--live';
    if (seismicTag) { seismicTag.textContent = 'LIVE'; seismicTag.className = 'source-item__tag'; }
    const panelDot = document.getElementById('seismic-panel-dot');
    if (panelDot) panelDot.className = 'status-dot status-dot--live';
  } else {
    if (seismicDot) seismicDot.className = 'status-dot status-dot--offline';
    if (seismicTag) { seismicTag.textContent = 'OFFLINE'; seismicTag.className = 'source-item__tag source-item__tag--sim'; }
    const panelDot = document.getElementById('seismic-panel-dot');
    if (panelDot) panelDot.className = 'status-dot status-dot--offline';
  }
}

function processSeismicData(features) {
  seismicMarkers.forEach(m => {
    layers.seismic.remove(m);
    const idx = pickableObjects.indexOf(m);
    if (idx > -1) pickableObjects.splice(idx, 1);
  });
  seismicMarkers.length = 0;
  seismicRings.forEach(({ ring }) => layers.seismic.remove(ring));
    ...
    if (liveSeismic.length > 0) {
      seismicListEl.innerHTML = liveSeismic.slice(0, 6).map(q =>
        `<div class="aircraft-item">
          ...
          <span class="aircraft-item__alt" style="font-size:9px;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${(q.place || 'Unknown').substring(0, 22)}</span>
        </div>`
      ).join('');
...
// ============================================================
// FIRMS_KEY = 'f6249be20191be88f901d1c7305350a0' (defined in Part 1)

// liveThermal declared at top of file
// thermalMarkers hoisted to top (TDZ fix)
...
async function fetchFIRMS() {
  const url = `https://firms.modaps.eosdis.nasa.gov/api/area/csv/${FIRMS_KEY}/MODIS_NRT/30,12,65,42/2`;
  try {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 15000);
    const res = await fetch(url, { signal: controller.signal });
    clearTimeout(timeout);
    if (!res.ok) throw new Error('FIRMS response not ok');
    const csvText = await res.text();
    processThermalData(csvText);
    setFIRMSStatus('live');
    return;
  } catch(e) {
    // fall through
...
function setFIRMSStatus(status) {
  if (status === 'live') {
    if (firmsDot) firmsDot.className = 'status-dot status-dot--live';
    if (firmsTag) { firmsTag.textContent = 'LIVE'; firmsTag.className = 'source-item__tag'; }
    const panelDot = document.getElementById('firms-panel-dot');
    if (panelDot) panelDot.className = 'status-dot status-dot--live';
  } else {
    if (firmsDot) firmsDot.className = 'status-dot status-dot--offline';
    if (firmsTag) { firmsTag.textContent = 'OFFLINE'; firmsTag.className = 'source-item__tag source-item__tag--sim'; }
    const panelDot = document.getElementById('firms-panel-dot');
    if (panelDot) panelDot.className = 'status-dot status-dot--offline';
  }
}
...
// gdeltMarkers hoisted to top (TDZ fix)
...
async function fetchGDELT() {
  ...
  // Try each query direct first, then via allorigins proxy
...
function setGDELTStatus(status) {
  if (status === 'live') {
    if (gdeltDot) gdeltDot.className = 'status-dot status-dot--live';
    if (gdeltTag) { gdeltTag.textContent = 'LIVE'; gdeltTag.className = 'source-item__tag'; }
  } else if (status === 'sim') {
    if (gdeltDot) gdeltDot.className = 'status-dot status-dot--sim';
    if (gdeltTag) { gdeltTag.textContent = 'SIM'; gdeltTag.className = 'source-item__tag source-item__tag--sim'; }
  } else {
    if (gdeltDot) gdeltDot.className = 'status-dot status-dot--offline';
    if (gdeltTag) { gdeltTag.textContent = 'OFFLINE'; gdeltTag.className = 'source-item__tag source-item__tag--sim'; }
  }
}

function processGDELTData(articles) {
  gdeltMarkers.forEach(m => {
    layers.gdelt.remove(m);
    const idx = pickableObjects.indexOf(m);
    if (idx > -1) pickableObjects.splice(idx, 1);
  });
  gdeltMarkers.length = 0;
...
function mergeGDELTIntoNewsFeed(gdeltItems) {
  ...
  gdeltItems.slice(0, 8).forEach(item => {
    ...
    a.innerHTML = `
      ...
      <div class="news-item__content">
        <div class="news-item__title">${item.title.substring(0, 100)}</div>
        <div class="news-item__meta">
          <span class="news-item__source" style="color:#00e5cc">GDELT / ${item.domain}</span>
...
function setSigmetStatus(status) {
  if (sigmetDot) sigmetDot.className = status === 'live' ? 'status-dot status-dot--live' : 'status-dot status-dot--offline';
  if (sigmetTag) {
    sigmetTag.textContent = status === 'live' ? 'LIVE' : 'OFFLINE';
    sigmetTag.className = status === 'live' ? 'source-item__tag' : 'source-item__tag source-item__tag--sim';
  }
}

async function fetchSIGMETs() {
  const directUrl = 'https://aviationweather.gov/api/data/isigmet?format=geojson';
  const proxyUrl = `https://api.codetabs.com/v1/proxy/?quest=${encodeURIComponent(directUrl)}`;
  // aviationweather.gov has no CORS headers — try proxy first, then direct
...
function processSIGMETData(features) {
  sigmetMarkers.forEach(m => {
    layers.sigmet.remove(m);
    const idx = pickableObjects.indexOf(m);
    if (idx > -1) pickableObjects.splice(idx, 1);
  });
  sigmetMarkers.length = 0;
  ...
  items.forEach(item => {
    ...
    a.innerHTML = `
      ...
      <div class="news-item__content">
        <div class="news-item__title">${(item.title || '').substring(0, 100)}</div>
        <div class="news-item__meta">
          <span class="news-item__source" style="color:${item.color}">${item.source}</span>
          <span class="news-item__time">${timeAgo}</span>
        </div>
      </div>
    `;
...
function mergeIDFIntoFeed(items) {
  ...
  items.forEach(item => {
    ...
    const timeAgo = getTimeAgo(item.pubDate);
    a.innerHTML = `
      <div class="news-item__dot" style="background:${item.color};border:1px solid rgba(255,255,255,0.3);"></div>
      <div class="news-item__content">
        <div class="news-item__title">${(item.title || '').substring(0, 100)}</div>
        <div class="news-item__meta">
          <span class="news-item__source" style="color:${item.color}">${item.isIDF ? '🇮🇱 ' : ''}${item.source}</span>
          <span class="news-item__time">${timeAgo}</span>
        </div>
      </div>
    `;
...
function updateOilPriceDisplay() {
  ...
  const oilChangeEl = document.getElementById('oil-price-change');
  ...
  if (oilChangeEl) {
    ...
    oilChangeEl.textContent = `${sign}$${oilPriceChange.toFixed(2)} (${sign}${((oilPriceChange / (oilPrice - oilPriceChange)) * 100).toFixed(1)}%)`;
    oilChangeEl.style.color = oilPriceChange >= 0 ? '#ef4444' : '#22c55e'; // red = up (bad for consumers)
  }
  if (oilStatusEl) {
...
async function fetchOilPrice() {
  // Primary: oilpriceapi.com demo endpoint (public, no CORS issue via proxy)
  const OIL_BASE = 'https://api.oilpriceapi.com/v1/demo/prices';
  const endpoints = [
    OIL_BASE,
    `https://api.allorigins.win/raw?url=${encodeURIComponent(OIL_BASE)}`,
    `https://api.codetabs.com/v1/proxy/?quest=${encodeURIComponent(OIL_BASE)}`,
  ];
  ...
  for (const url of endpoints) {
    try {
      ...
      // Extract Brent crude price — demo endpoint returns {status, data: {prices: [{code, price}...]}}
      let price = null;
      if (data?.data?.prices && Array.isArray(data.data.prices)) {
        const brent = data.data.prices.find(p => p.code === 'BRENT_CRUDE_USD');
      ...
      if (price && !isNaN(price) && price > 10 && price < 500) {
...
console.log('[SIGINT-MAP] Sections 33–35 loaded: Oil Price Ticker, Search, Audio Alerts');
console.log('[SIGINT-MAP] Audio system: Web Audio API (no external files). Click sound toggle to enable.');
// ============================================================
...
async function fetchAdsbLol() {
  ...
  const urls = [
    ...
    BASE_URL,
    `https://api.codetabs.com/v1/proxy/?quest=${encodeURIComponent(BASE_URL)}`,
  ];

  const adsbDot = document.getElementById('adsb-lol-src-dot');
  const adsbTag = adsbDot?.parentElement?.querySelector('.source-item__tag');
  ...
  for (const url of urls) {
    try {
      ...
      if (data && Array.isArray(data.ac)) {
        ...
        if (adsbDot) adsbDot.className = 'status-dot status-dot--live';
        if (adsbTag) { adsbTag.textContent = `LIVE (${data.ac.length})`; adsbTag.className = 'source-item__tag'; }
        return;
      }
    } catch (e) { continue; }
  }
  ...
  if (adsbDot) adsbDot.className = 'status-dot status-dot--live';
  if (adsbTag) { adsbTag.textContent = `SIM (${simAC.length})`; adsbTag.className = 'source-item__tag source-item__tag--sim'; }
}

function processAdsbLolData(ac) {
  // --- Clear old markers ---
  adsbMilMarkers.forEach(m => {
    layers.adsbMilitary.remove(m);
    const idx = pickableObjects.indexOf(m);
    if (idx > -1) pickableObjects.splice(idx, 1);
  });
  adsbMilMarkers.length = 0;
  ...
  ac.forEach(a => {
    if (!a.lat || !a.lon) return;                     // no position
  ...
  console.log(`[ADSB.lol] ${adsbMilAircraft.length} military aircraft plotted`);
}
...
console.log('[SIGINT-MAP] Section 36 loaded: ADSB.lol Military Flight Tracking');
...
async function fetchMaritime() {
  ...
  if (maritimeStatusEl) maritimeStatusEl.innerHTML =
    `<span class="status-dot status-dot--sim"></span> SIM (${vesselCount})`;
  if (maritimeCountEl) maritimeCountEl.textContent = vesselCount;
}

function processMaritime(features) {
  // Clear previous markers
  maritimeMarkers.forEach(m => {
    layers.maritime.remove(m);
    const idx = pickableObjects.indexOf(m);
    if (idx > -1) pickableObjects.splice(idx, 1);
  });
  maritimeMarkers.length = 0;
  ...
  console.log(`[Maritime] ${maritimeVessels.length} vessels plotted`);
}

fetchMaritime();
setInterval(fetchMaritime, MARITIME_INTERVAL);

console.log('[SIGINT-MAP] Section 37 loaded: Maritime Vessel Tracking (AIS)');
...
async function fetchWeather() {
  // Build a multi-location request (Open-Meteo supports
  // comma-separated lat/lon arrays)
  ...
  const windSrcDots = document.querySelectorAll('.source-item');
  let windStatusEl = null;
  windSrcDots.forEach(el => { if (el.textContent.includes('Open-Meteo Wind')) windStatusEl = el; });
  ...
  for (const url of urls) {
    try {
      ...
      if (responses.length > 0 && responses[0].current) {
        ...
        if (windStatusEl) {
          windStatusEl.querySelector('.status-dot').className = 'status-dot status-dot--live';
          const tag = windStatusEl.querySelector('.source-item__tag');
          if (tag) { tag.textContent = 'LIVE'; tag.className = 'source-item__tag'; }
        }
        return;
      }
    } catch (e) { continue; }
  ...
  if (windStatusEl) {
    windStatusEl.querySelector('.status-dot').className = 'status-dot status-dot--offline';
    const tag = windStatusEl.querySelector('.source-item__tag');
    if (tag) { tag.textContent = 'OFFLINE'; tag.className = 'source-item__tag source-item__tag--sim'; }
  }
...
function processWeatherData(responses) {
  ...
  weatherArrows.forEach(obj => {
    layers.weather.remove(obj);
  });
  weatherArrows.length = 0;
  ...
  console.log(`[Weather] Wind data updated for ${summaries.length} locations`);
}

fetchWeather();
setInterval(fetchWeather, WEATHER_INTERVAL);

console.log('[SIGINT-MAP] Section 38 loaded: Open-Meteo Wind Visualization');
...
async function fetchRadiation() {
  ...
  const safecastUrls = [
    SAFECAST_URL,
    `https://api.allorigins.win/raw?url=${encodeURIComponent(SAFECAST_URL)}`,
    `https://api.codetabs.com/v1/proxy/?quest=${encodeURIComponent(SAFECAST_URL)}`,
  ];

  for (const url of safecastUrls) {
    try {
      const controller = new AbortController();
      ...
      arr.forEach(m => {
        const lat   = parseFloat(m.latitude)  || parseFloat(m.lat);
        const lon   = parseFloat(m.longitude) || parseFloat(m.lon);
        const value = parseFloat(m.value);     // stored in CPM by default
        if (!lat || !lon || isNaN(value)) return;

        // Safecast stores in CPM — convert to µSv/h (rough: CPM / 334 for Cs-137)
      ...
      if (readings.length > 0) break; // success
    } catch (e) {
      continue;
    }
  }

  // ---- Source 2: OpenRadiation ----
  const OPENRAD_URL =
    'https://request.openradiation.net/measurements' +
    '?apiKey=bde8ebc61cb089b8cc997dd7a0d0a434';

  const openRadUrls = [
    OPENRAD_URL,
    `https://api.allorigins.win/raw?url=${encodeURIComponent(OPENRAD_URL)}`,
    `https://api.codetabs.com/v1/proxy/?quest=${encodeURIComponent(OPENRAD_URL)}`,
  ];

  for (const url of openRadUrls) {
    try {
      const controller = new AbortController();
      const timeout = setTimeout(() => controller.abort(), 9000);
      const res = await fetch(url, { signal: controller.signal });
      ...
      arr.forEach(m => {
        const lat   = parseFloat(m.latitude)  || parseFloat(m.lat);
        const lon   = parseFloat(m.longitude) || parseFloat(m.lon);
        const usvh  = parseFloat(m.value);    // OpenRadiation uses µSv/h natively
        if (!lat || !lon || isNaN(usvh)) return;

        // Accept global readings — radiation monitoring is worldwide
        // Show all readings with preference for ME region
      ...
      break; // success or empty — no need to try proxy if we got a response
    } catch (e) {
      continue;
    }
  }
...
function processRadiationData(readings) {
  ...
  radiationMarkers.forEach(m => {
    layers.radiation.remove(m);
    const idx = pickableObjects.indexOf(m);
    if (idx > -1) pickableObjects.splice(idx, 1);
  });
  radiationMarkers.length = 0;
  ...
  if (radiationStatusEl) radiationStatusEl.innerHTML =
    `<span class="status-dot status-dot--live"></span> ${readings.length} readings`;

  console.log(`[Radiation] ${readings.length} readings plotted`);
}
...
console.log('[SIGINT-MAP] Section 39 loaded: Safecast + OpenRadiation Monitoring');
...
async function fetchUCDP() {
  ...
  const urls = [
    RW_URL,
    `https://api.allorigins.win/raw?url=${encodeURIComponent(RW_URL)}`,
  ];
  ...
  for (const url of urls) {
    try {
      ...
      if (items.length > 0) {
        // Convert ReliefWeb format to UCDP-like events
        const events = items.map(item => {
          ...
          return {
            ...
            best: 0,
            side_a: f.source?.map(s => s.name).join(', ') || 'Unknown',
            side_b: 'Unknown',
          };
        });
        isUCDPLive = true;
  ...
  processUCDP(events);
  console.log('[UCDP] Using conflict zone simulation (12 active zones)');
}

function processUCDP(events) {
  // Clear old markers
  conflictMarkers.forEach(m => {
    layers.conflict.remove(m);
    const idx = pickableObjects.indexOf(m);
    if (idx > -1) pickableObjects.splice(idx, 1);
  });
  ...
  if (conflictStatusEl) {
    const dotClass = isUCDPLive ? 'status-dot--live' : 'status-dot--sim';
    const label = isUCDPLive ? '' : 'SIM ';
    conflictStatusEl.innerHTML =
      `<span class="status-dot ${dotClass}"></span> ${label}${conflictEvents.length} events`;
  }

  console.log(`[UCDP] ${conflictEvents.length} conflict events plotted`);
}

fetchUCDP();
setInterval(fetchUCDP, UCDP_INTERVAL);

console.log('[SIGINT-MAP] Section 40 loaded: UCDP Conflict Event Data');
...
async function fetchMarineWeather() {
  ...
  const marineSrcDots = document.querySelectorAll('.source-item');
  let marineStatusSrc = null;
  marineSrcDots.forEach(el => { if (el.textContent.includes('Marine Weather')) marineStatusSrc = el; });
  ...
  for (const url of urls) {
    try {
      ...
      if (responses.length > 0 && responses[0].current) {
        ...
        if (marineStatusSrc) {
          marineStatusSrc.querySelector('.status-dot').className = 'status-dot status-dot--live';
          const tag = marineStatusSrc.querySelector('.source-item__tag');
          if (tag) { tag.textContent = 'LIVE'; tag.className = 'source-item__tag'; }
        }
        return;
      }
  ...
  if (marineStatusSrc) {
    marineStatusSrc.querySelector('.status-dot').className = 'status-dot status-dot--offline';
    const tag = marineStatusSrc.querySelector('.source-item__tag');
    if (tag) { tag.textContent = 'OFFLINE'; tag.className = 'source-item__tag source-item__tag--sim'; }
  }
...
function processMarineWeather(responses) {
  ...
  marineRings.forEach(obj => layers.marine.remove(obj));
  marineRings.length = 0;
  marineReadings = [];
  ...
  if (marineStatusEl) marineStatusEl.innerHTML =
    `<span class="status-dot status-dot--live"></span> ${marineReadings.length} pts`;

  console.log(`[Marine] Wave data updated: max ${maxWave.toFixed(2)}m`);
}
...
console.log('[SIGINT-MAP] Section 41 loaded: Open-Meteo Marine Weather (Hormuz)');
...
function updateAnalyticsDashboard() {
  try {
    ...
    const openskyCount  = layers.aircraft      ? layers.aircraft.children.length      : 0;
    const adsbMilCount  = layers.adsbMilitary  ? layers.adsbMilitary.children.length  : 0;
    const totalAircraft = openskyCount + adsbMilCount;

    // --- Vessels ---
    const totalVessels  = layers.maritime      ? layers.maritime.children.length       : 0;

    // --- Conflict events: GDELT (layers.gdelt) + UCDP (layers.conflict) ---
    const gdeltCount    = layers.gdelt         ? layers.gdelt.children.length          : 0;
    const ucdpCount     = layers.conflict      ? layers.conflict.children.length       : 0;
    const totalConflict = gdeltCount + ucdpCount;

    // --- Thermal hotspots ---
    const totalThermal  = layers.thermal       ? layers.thermal.children.length        : 0;

    // --- Seismic events ---
    const totalSeismic  = layers.seismic       ? layers.seismic.children.length        : 0;
    ...
    if (layers.weather && layers.weather.children.length > 0) {
      // Read from head userData of weather arrows
      const heads = [];
      layers.weather.children.forEach(grp => {
        grp.children && grp.children.forEach(child => {
          if (child.userData && child.userData.isWeather) heads.push(child);
        });
      });
      if (heads.length > 0) {
        // Parse wind speed from detail string (e.g. "Speed: 45 km/h")
        const speeds = heads.map(h => {
          const m = (h.userData.detail || '').match(/Speed:\s*([\d.]+)/);
...
console.log('[SIGINT-MAP] Section 42 loaded: Extended Analytics Dashboard');
...
document.querySelectorAll('.filter-btn').forEach(btn => {
  ...
  newBtn.addEventListener('click', () => {
    ...
    currentFilter = newBtn.dataset.filter;
    applyFilter();
    // Re-apply search if the search system is active (defined in Section 34)
...
console.log('[SIGINT-MAP] Section 43 loaded: Enhanced Filter System (all layers)');
...
function updateAllSourceStatuses() {
  ...
  items.forEach(el => {
    ...
    if (label.includes('Radiation')) {
      ...
      // Use isUCDPLive flag — markers exist even with simulated data
      isSim = !isUCDPLive && conflictMarkers.length > 0;
      isLive = isUCDPLive && conflictMarkers.length > 0;
      count = (isLive || isSim) ? ` (${conflictMarkers.length})` : '';
    ...
    } else if (label.includes('Maritime AIS')) {
      // Maritime is 100% simulated — always show SIM
      isSim = maritimeMarkers.length > 0;
      count = isSim ? ` (${maritimeMarkers.length})` : '';
    } else if (label.includes('Oil Price')) {
      // Use isOilPriceLive flag instead of hardcoded true
      isLive = isOilPriceLive;
      isSim = !isOilPriceLive;
    } else {
    ...
    if (isLive) {
      ...
      tag.textContent = 'LIVE' + count;
      tag.className = 'source-item__tag';
    } else if (isSim) {
      dot.className = 'status-dot status-dot--sim';
      tag.textContent = 'SIM' + count;
...
// ============================================================
console.log('[SIGINT-MAP] Sections 36–43 loaded: All new API integrations active');
// ============================================================
...
console.log('[S44] Section 44 initializing — Native Browser Web API Enhancements');
...
(function nw_initPageVisibility() {
  ...
  function onVisibilityChange() {
    if (document.hidden) {
      ...
      console.log('[S44] Tab hidden — pausing data fetches');
      ...
      console.log('[S44] Tab visible — resuming; burst-refresh triggered');
    }
  }

  document.addEventListener('visibilitychange', onVisibilityChange);
  console.log('[S44] ✓ Page Visibility API active');
})();
...
function nw_burstRefresh() {
  ...
  console.log('[S44] Burst-refresh: re-fetching all sources');
...
(function nw_initNetworkInfo() {
  ...
  function evaluateConnection() {
    ...
    if (conn) {
      ...
      console.log(`[S44] Connection: type=${eff}, downlink=${dl}Mbps, RTT=${conn.rtt}ms → quality=${quality}`);
    }
  ...
  function nw_applyBandwidthMode() {
    ...
    if (isLow) {
      ...
      console.log('[S44] Low bandwidth mode: reduced fetch frequencies & render quality');
    } else {
  ...
  window.addEventListener('online', () => {
    ...
    console.log('[S44] Online — burst-refreshing all sources');
    nw_analytics.connectionChanges++;
    evaluateConnection();
  ...
  if (conn) {
    ...
    console.log('[S44] ✓ Network Information API available');
  } else {
  ...
  console.log('[S44] ✓ Network detection active');
})();
  ...
  document.addEventListener('pointerdown', () => {
    if (!nw_permRequested && Notification.permission === 'default') {
      ...
      Notification.requestPermission().then(p => {
        console.log(`[S44] Notification permission: ${p}`);
      });
    }
  }, { once: true });

  console.log('[S44] ✓ Web Notifications API initialized');
})();
...
`;
  ...
  try {
    ...
    nw_worker.addEventListener('message', (e) => {
      ...
      if (id === '__ready__') {
        nw_workerReady = true;
        console.log('[S44] ✓ Web Worker ready');
    ...
    console.log('[S44] Web Worker created, awaiting ready signal…');
  } catch(e) {
...
function nw_workerTask(task, payload) {
  ...
  // Fallback: run synchronously (minimal, best-effort)
  console.warn('[S44] Worker unavailable — running task inline:', task);
  return Promise.resolve(payload.items || payload.states || payload.features || []);
}
...
(function nw_initWakeLock() {
  ...
  console.log('[S44] ✓ Screen Wake Lock button created');
})();
...
async function nw_acquireWakeLock() {
  ...
  try {
    ...
    nw_updateWakeLockUI();
    console.log('[S44] Wake Lock acquired');

    nw_wakeLock.addEventListener('release', () => {
      nw_wakeLockActive = false;
      nw_updateWakeLockUI();
      console.log('[S44] Wake Lock released');
    });
  } catch(e) {
...
(function nw_initGeolocation() {
  ...
  console.log('[S44] ✓ Geolocation button created');
})();
...
function nw_geolocateUser() {
  ...
  navigator.geolocation.getCurrentPosition(
    (pos) => {
      ...
      console.log(`[S44] User position: ${lat.toFixed(4)}, ${lon.toFixed(4)} (±${Math.round(accuracy)}m)`);
...
(function nw_initClipboard() {
  ...
  console.log('[S44] ✓ Clipboard / Share button created');
})();
...
async function nw_copyBriefing() {
  ...
  try {
    ...
    console.log('[S44] Briefing copied to clipboard');
  } catch(e) {
...
(function nw_initPerformance() {
  ...
  if ('PerformanceObserver' in window) {
    try {
      ...
      console.log('[S44] ✓ Long task observer active');
    } catch(e) {
    ...
    console.log('[S44] ✓ Performance Observer initialized');
  } else {
    console.warn('[S44] PerformanceObserver not supported');
  }

  console.log('[S44] ✓ Performance overlay created (press P to toggle)');
})();
...
function nw_togglePerfOverlay() {
  ...
  console.log(`[S44] Performance overlay: ${nw_perfOverlayVisible ? 'ON' : 'OFF'}`);
}
...
(function nw_initIntersectionObserver() {
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      ...
      if (entry.isIntersecting) {
        ...
        if (data.paused) {
          data.paused = false;
          console.log(`[S44] Sidebar section visible: ${entry.target.id} — resuming updates`);
        ...
        if (!data.paused) {
          data.paused = true;
          console.log(`[S44] Sidebar section hidden: ${entry.target.id} — pausing updates`);
        }
      }
    });
  }, {
  ...
  console.log(`[S44] ✓ Intersection Observer watching ${nw_sidebarObservers.size} sidebar sections`);
})();
...
(function nw_initBroadcastChannel() {
  ...
  function electPrimary() {
    ...
    if (nw_isPrimary) {
      console.log('[S44] This tab elected as PRIMARY data fetcher');
      nw_tabNumber = 1;
    } else {
  ...
  nw_bc.addEventListener('message', (e) => {
    ...
    switch (type) {
      ...
      case 'STATE_SYNC':
        ...
        if (isPrimary && filter && typeof applyFilter === 'function') {
          try {
            if (typeof currentFilter !== 'undefined' && currentFilter !== filter) {
              ...
              if (typeof currentFilter !== 'undefined') {
                // We use a workaround since currentFilter is let in outer scope
                window._nw_syncFilter = filter;
                applyFilter();
              }
            }
          } catch(e) {}
      ...
      case 'DATA_UPDATE':
        ...
        console.log(`[S44] Received ${e.data.dataType} data from primary tab`);
        break;
  ...
  console.log(`[S44] ✓ Broadcast Channel initialized (tabId: ${nw_tabId})`);
})();
...
(function nw_initBeacon() {
  ...
  window.addEventListener('visibilitychange', () => {
    ...
    if (document.hidden) {
      ...
      console.log('[S44] SESSION BEACON (visibilitychange→hidden):', payload);
      ...
      if ('sendBeacon' in navigator) {
        ...
        console.log(`[S44] Beacon payload size: ${body.length} bytes`);
      }
    }
  });
  ...
  window.addEventListener('beforeunload', () => {
    const payload = buildSessionPayload();
    const body    = JSON.stringify(payload);
    console.log('[S44] SESSION BEACON (beforeunload):', payload);
    // navigator.sendBeacon('/api/session', body);
    if ('sendBeacon' in navigator) {
      console.log(`[S44] Final beacon: ${body.length} bytes ready to transmit`);
    }
  });
  ...
  setInterval(() => {
    const payload = buildSessionPayload();
    console.log('[S44] Periodic session snapshot:', payload);
  }, 60000);

  console.log('[S44] ✓ Beacon API + Session Analytics initialized');
})();
  ...
  nw_scheduleIdle((deadline) => {
    ...
    for (const key of layerKeys) {
      ...
      try {
        ...
        toRemove.forEach(m => layer.remove(m));
        if (toRemove.length > 0) {
          console.log(`[S44] Idle dedup: removed ${toRemove.length} duplicates from ${key}`);
        }
      } catch(e) {}
    }
  }, { timeout: 8000 });
...
console.log('[S44] ✓ requestIdleCallback scheduler initialized');
...
(function nw_initKeyboardShortcuts() {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    switch (e.key) {
      ...
      case 'm': case 'M': {
        ...
        console.log(`[S44] Low-bandwidth mode: ${nw_lowBandwidthManual ? 'ON' : 'OFF'} (manual toggle)`);
        break;
      }
      ...
      default: {
        if (e.key >= '1' && e.key <= '9') {
          ...
          if (filter && typeof applyFilter === 'function') {
            ...
            console.log(`[S44] Preset key ${e.key}: filter → "${filter}"`);
          }
        }
        break;
      }
  ...
  console.log('[S44] ✓ Keyboard shortcuts enhanced (N/P/L/G/C/M/1-9/?)');
})();

// ─────────────────────────────────────────────────────────────
// NW-14: DATA SOURCE TRACKING (SUCCESS / FAILURE HOOKS)
// ─────────────────────────────────────────────────────────────
// Wrap each fetch function to track success/failure in analytics
// and trigger worker processing for heavy responses.

(function nw_wrapFetchFunctions() {

  // Helper: wrap an async function to track analytics + worker offload
  function wrapFetch(name, originalFnGetter, workerTask, workerPayloadFn) {
    return async function nw_wrapped(...args) {
      const original = originalFnGetter();
      if (typeof original !== 'function') return;

      // Check if paused
      if (nw_tabPaused || !navigator.onLine) {
        console.log(`[S44] Skipping ${name} — tab paused or offline`);
        return;
      }

      try {
        const result = await original.apply(this, args);
        nw_analytics.dataSourcesOk.add(name);

        // Offload processing to worker if applicable
        if (workerTask && nw_workerReady && result) {
          const payload = workerPayloadFn ? workerPayloadFn(result) : result;
          nw_workerTask(workerTask, payload)
            .then(processed => {
              console.log(`[S44] Worker processed ${name}: ${
                Array.isArray(processed) ? processed.length + ' items' : typeof processed}`);
            })
            .catch(e => console.warn(`[S44] Worker error for ${name}:`, e));
        }

        return result;
      } catch(e) {
        nw_analytics.dataSourcesFailed.add(name);
        throw e;
      }
    };
  }

  // We don't replace the original functions (they run as-is),
  // but we hook into data-ready events via periodic analytics checks.
  // The worker is called explicitly for heavy payloads via nw_processRawData().

  console.log('[S44] ✓ Data source analytics hooks installed');
})();

/**
 * Called externally to process a raw API response through the worker.
 * @param {string} task     — worker task name
 * @param {object} payload  — raw API data
 * @returns {Promise<any>}
 */
function nw_processRawData(task, payload) {
  return nw_workerTask(task, payload);
}
...
(function nw_injectStyles() {
  ...
  console.log('[S44] ✓ Section 44 styles injected');
})();
...
(function nw_initMonitoringOrchestrator() {
  ...
  console.log('[S44] ✓ Monitoring orchestrator active');
})();
...
(function nw_initNotificationTriggers() {
  ...
  console.log('[S44] ✓ Notification trigger scanner active (15s interval)');
})();

// ─────────────────────────────────────────────────────────────
// NW-18: WORKER DATA PIPELINE — OpenSky INTEGRATION HELPER
// ─────────────────────────────────────────────────────────────
// Provides a helper that existing fetch functions can call to
// offload their heavy parsing to the worker.

/**
 * Filter OpenSky states to AOR via worker.
 * @param {object} rawResponse — raw OpenSky API JSON
 * @returns {Promise<Array>}   — lean, filtered aircraft array
 */
async function nw_filterOpenSkyData(rawResponse) {
  if (!rawResponse || !rawResponse.states) return [];
  try {
    return await nw_workerTask('filterOpenSky', { states: rawResponse.states });
  } catch(e) {
    console.warn('[S44] OpenSky worker filter failed:', e);
    return rawResponse.states || [];
  }
}

/**
 * Filter USGS features to AOR via worker.
 * @param {object} rawResponse — raw USGS GeoJSON
 * @returns {Promise<Array>}
 */
async function nw_filterSeismicData(rawResponse) {
  if (!rawResponse || !rawResponse.features) return [];
  try {
    return await nw_workerTask('filterSeismic', { features: rawResponse.features });
  } catch(e) {
    return rawResponse.features || [];
  }
}

/**
 * Score news items for sentiment via worker.
 * @param {Array} items
 * @returns {Promise<Array>}
 */
async function nw_scoreNewsData(items) {
  if (!items || !items.length) return items;
  try {
    return await nw_workerTask('scoreNews', { items });
  } catch(e) {
    return items;
  }
}
...
/**
 * Deduplicate events via worker.
 * @param {Array} items
 * @param {number} windowMs
 * @returns {Promise<Array>}
 */
async function nw_deduplicateEvents(items, windowMs = 300000) {
  if (!items || !items.length) return items;
  try {
    return await nw_workerTask('deduplicateEvents', { items, windowMs });
  } catch(e) {
    return items;
  }
}
...
/**
 * Returns the recommended fetch interval multiplier based on bandwidth.
 * Low/medium bandwidth → 3× longer intervals.
 */
function nw_fetchIntervalMultiplier() {
  if (nw_lowBandwidthManual) return 3;
  if (nw_connectionQuality === 'low')    return 3;
  if (nw_connectionQuality === 'medium') return 2;
  return 1;
}
...
console.log('[S44] ✓ Section 44: Native Browser Web API Enhancements — ALL SYSTEMS ACTIVE');
console.log('[S44]   APIs: PageVisibility | NetworkInfo | Notifications | Worker | WakeLock');
console.log('[S44]         Geolocation | Clipboard/Share | PerformanceObserver | IntersectionObserver');
console.log('[S44]         BroadcastChannel | Beacon | requestIdleCallback | KeyboardEvents');
console.log('[S44]   Run window.nw_diagnostics() in console for live status report');
