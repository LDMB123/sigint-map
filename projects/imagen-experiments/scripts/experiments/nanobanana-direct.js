...
/**
 * Nano Banana Pro - Vertex AI Implementation
 * Gemini 3 Pro Image Preview via Vertex AI with OAuth
 * All advanced features: 4K, reference images, grounding, thinking, editing
 */
...
const ENV_PATH = process.env.VERTEX_ENV_PATH || path.join(PROJECT_ROOT, 'config', 'vertex.env');
...
async function loadEnvFile(filePath) {
  try {
    ...
    if (error && error.code !== 'ENOENT') {
      console.warn(`Warning: Failed to read env file at ${filePath}: ${error.message}`);
    }
  }
}
...
const PROJECT_ID = process.env.GOOGLE_CLOUD_PROJECT || process.env.GCP_PROJECT_ID || 'gen-lang-client-0925343693';
 ...
 * @param {string} options.editImagePath - Optional: path to image to edit
 * @param {boolean} options.enableGoogleSearch - Optional: enable Google Search grounding
 * @param {boolean} options.showThinkingProcess - Optional: display thinking steps (default: true)
...
async function generateImage(options = {}) {
  ...
  console.log(`\n🎨 ${editImagePath ? 'Editing' : 'Generating'} with Nano Banana Pro (Vertex AI)...`);
  ...
  if (savedImages.length === 0) {
    ...
    console.warn('Example: DEBUG_RESPONSE=true node nanobanana-vertex.js ...');
  }
  ...
  try {
    if (command === 'generate' && prompt) {
      ...
      if (!imagePath || !editPrompt) {
        console.error('Usage: node nanobanana-vertex.js edit /path/to/image.jpg "add flowers"');
        process.exit(1);
      }
      await generateImage({
        prompt: editPrompt,
      ...
      if (!promptText || refImages.length === 0) {
        console.error('Usage: node nanobanana-vertex.js with-ref "prompt" ref1.jpg ref2.jpg');
        process.exit(1);
      }
...
Auth: Google Cloud OAuth (application_default_credentials.json)
Project: ${PROJECT_ID}

Setup:
  1. Authenticate: gcloud auth application-default login
  2. Set project: export GOOGLE_CLOUD_PROJECT="${PROJECT_ID}"

Usage:
  Generate:  node nanobanana-direct.js generate "business card with 'ACME Corp'"
  Grounded:  node nanobanana-direct.js grounded "infographic about climate change"
  Edit:      node nanobanana-direct.js edit image.jpg "add flowers"
