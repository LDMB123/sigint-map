...
export RATE_LIMIT_RETRY_MAX_S="${RATE_LIMIT_RETRY_MAX_S:-180}"
export RATE_LIMIT_COOLDOWN_BASE_S="${RATE_LIMIT_COOLDOWN_BASE_S:-61}"
export RATE_LIMIT_COOLDOWN_MAX_S="${RATE_LIMIT_COOLDOWN_MAX_S:-180}"
export RATE_LIMIT_COOLDOWN_GROWTH="${RATE_LIMIT_COOLDOWN_GROWTH:-1.35}"
...
export RATE_LIMIT_PRESSURE_DEGRADED_ACCEPT="${RATE_LIMIT_PRESSURE_DEGRADED_ACCEPT:-1}"
...
export MICRODETAIL_PROFILE_LOCK="${MICRODETAIL_PROFILE_LOCK:-1}"
export EXPECT_MICRODETAIL_PROFILE="${EXPECT_MICRODETAIL_PROFILE:-1}"

if [[ "$MICRODETAIL_PROFILE_LOCK" == "1" ]]; then
  export MICRO_PHYSICS_LANGUAGE_ENFORCEMENT=1
  export MICRO_PHYSICS_BANNED_TERMS=strict
  export FIRST_PRINCIPLES_RECOMPILER_MODE=1
  export PROMPT_DIRECTION_SUPREMACY_MODE=1
  export ENABLE_RESEARCH_MICRODETAIL_EXPANSION=1
  export TARGETED_MICRODETAIL_MODE=1
  export PHYSICS_DENSITY_MULTIPLIER="${PHYSICS_DENSITY_MULTIPLIER:-5}"
  export PHYSICS_DENSITY_MIN_RATIO="${PHYSICS_DENSITY_MIN_RATIO:-5.0}"
fi
...
# Shed optional fallback requests under active 429 pressure.
...
# Enable variance controls so each prompt keeps distinct outfit architecture/color.
...
printf "Profile guard   : lock=%s expect=%s microLang=%s bannedTerms=%s\n" "$MICRODETAIL_PROFILE_LOCK" "$EXPECT_MICRODETAIL_PROFILE" "$MICRO_PHYSICS_LANGUAGE_ENFORCEMENT" "$MICRO_PHYSICS_BANNED_TERMS"
...
self_heal_runtime_knobs() {
  ...
  if (( rate_limit_failures > 0 )); then
    ...
    if [[ "$RATE_LIMIT_ADAPTIVE_COOLDOWN" != "1" ]]; then
      RATE_LIMIT_ADAPTIVE_COOLDOWN=1
      export RATE_LIMIT_ADAPTIVE_COOLDOWN
      changed=1
    fi
    if [[ "$RATE_LIMIT_SKIP_SAFE_FALLBACK_ON_PRESSURE" != "1" ]]; then
      RATE_LIMIT_SKIP_SAFE_FALLBACK_ON_PRESSURE=1
      export RATE_LIMIT_SKIP_SAFE_FALLBACK_ON_PRESSURE
      changed=1
    fi
    if [[ "$RATE_LIMIT_PRESSURE_DEGRADED_ACCEPT" != "1" ]]; then
      RATE_LIMIT_PRESSURE_DEGRADED_ACCEPT=1
      export RATE_LIMIT_PRESSURE_DEGRADED_ACCEPT
      changed=1
    fi
