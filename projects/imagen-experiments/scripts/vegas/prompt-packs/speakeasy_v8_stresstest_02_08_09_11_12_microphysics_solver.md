# Vegas Speakeasy v8 Stress Test (Targeted Weak IDs, Micro-Physics Solver)

Design goal: validate the highest-risk prompts after the micro-physics solver rewrite before full-batch execution.
Targets: 02, 08, 09, 11, 12.

v8 global delta (apply to all prompts, both primary and safe):
- Replace prose-heavy physics language with enforceable causality chains: pose/load -> strain -> fold orientation -> highlight elongation -> penumbra.
- Require zone-ledger realism: top support edge, waist transition, slit boundary, thigh-band, knee, ankle/instep, heel-floor, hand-contact.
- At each zone, require at least 2-3 physically coherent cues (strain gradient, seam relief, compression shadow, anisotropic highlight flow, denier gradient, friction-damped drape).
- Keep non-explicit framing and adult editorial context while maximizing contour clarity through physically plausible mechanics.
- Reject pseudo-realism where folds, highlights, and shadows are not causally coupled.
