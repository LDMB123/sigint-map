...
/**
 * V19 APEX - Two-pass, multi-turn refinement for max photorealism
 *
...
const OUTPUT_DIR = process.env.OUTPUT_DIR || path.join(process.env.HOME, 'nanobanana-output', 'vegas-v19-apex');
...
const LIGHT_BLOCK = `3D GLOBAL ILLUMINATION LIGHT TRANSPORT: Primary overhead recessed tungsten halogen track spots at 2800K creating hard directional pools with sharp penumbral shadow edges - NOT diffused fashion lighting. Secondary projection-mapped vault door at ~4300K casts soft geometric color spill with crisp boundaries following inverse-square falloff I=Phi/(4*pi*r^2). Tertiary LED mesh bottle wall at 3-stop underexposure as faint cool fill. Steep 4-stop luminance gradient from bar surface to deep booths. NO supplemental fill light - deep unrecoverable shadows on shadow-side at 5+ stops below key creating true black zero detail. Mixed color temperature: 2800K tungsten warm on skin vs 4300K cool projection contamination. Multi-bounce warm color interreflection from walnut bar surface adds ~300K to indirect shadow fill. Water-channel caustic projection on bar lip from overhead spot refraction through shallow ripples. Ambient occlusion darkening at body-bar contact zones. Volumetric light scatter through atmospheric haze particles.`;
...
const IMPERFECTIONS_BLOCK = `RAW PHOTOGRAPHIC IMPERFECTIONS: ISO 3200 grain visible in shadows. Motion blur fingertips 1/125s. Flyaway hair catching backlight as soft bright streaks. Background projection bokeh as large soft colored discs. Foreground glass edge refraction blur. Faint lens flare veiling glare and hexagonal ghost from brightest projection highlight. Crumpled napkin bar surface. Condensation ring cold glass. Micro dust shadow upper corner as faint dark circle. Barrel distortion pulling straight lines at edges. Chromatic aberration: lateral CA 0.5px red-cyan fringe at frame edges on high-contrast boundary. Sensor bloom: bright specular highlights bleeding 2-3px beyond physical boundary into adjacent pixels. Acoustic dampener vibration: faint micro-shake from mirror mechanism creating 0.5px directional smear on point sources. No retouching no color grading - straight RAW with WB only. Preserve face identical.`;

const SCENE_BASE = `EVENT-DRIVEN SPEAKEASY SETTING (FIRST PRINCIPLES): Obsidian Skybridge speakeasy hidden above the Strip. Glass floor reveals neon city light below. Black marble bar with mirrored underlip; smoked mirror ceiling; brass lattice truss overhead; projection-mapped vault door throwing slow geometric patterns; LED mesh bottle wall; suspended candle cylinders; shallow water channel behind bar generating caustic flicker; low haze in the room.`;
...
SCENE ACCENT: ${concept.scene}

GARMENT GEOMETRY (COVERAGE + CUTOUTS): ${concept.geometry}
GARMENT MATERIAL CLASS (MACRO PHYSICS): ${concept.material}
...
MATERIAL MICROSTRUCTURE (MICRO PHYSICS): ${concept.micro}
...
function validatePrompt(prompt, label) {
  ...
  if (!lower.includes('40-42cm') && !lower.includes('40cm')) {
    console.log(`WARN: ${label} missing explicit hemline 40-42cm anchor.`);
  }
...
const concepts = [
  {
    name: '151-electrowetting-cascade-open-sides',
    scene: 'Kinetic brass iris sculpture above the bar rotates slowly, throwing moving highlights.',
    geometry: 'Black satin microdress with a high halter neckline, fully open back to the waist, and extreme side cutouts bridged by micro-chain links at the ribcage and hip. Upper-thigh hemline 40-42cm.',
    ...
    hosiery: 'HOSIERY: Black thigh-high stockings with gunmetal welt. 8-denier T=0.90.',
  },
  {
    name: '152-magnetoelastic-rib-cage',
    scene: 'Projection-mapped vault door pulses with slow geometric bands.',
    geometry: 'Sculpted microdress with open back to the waist and vertical side cutouts up to the ribcage; narrow underbust aperture. Upper-thigh hemline 40-42cm.',
    ...
    hosiery: 'HOSIERY: Black thigh-high stockings with graphite welt. 8-denier T=0.90.',
  },
  {
    name: '153-acousto-optic-fringe-halter',
    scene: 'Low haze carries thin beams from the LED mesh bottle wall.',
    geometry: 'Halter microdress with a fully open back and high side cutouts; crystal fringe at hemline. Upper-thigh hemline 40-42cm.',
    ...
    hosiery: 'HOSIERY: Black thigh-high stockings with crystal welt. 8-denier T=0.90.',
  },
  {
    name: '154-liquid-crystal-elastomer-slit',
    scene: 'Smoked mirror ceiling doubles candle cylinders into repeating rows.',
    geometry: 'One-shoulder microdress with a diagonal front slit and open back to the waist; narrow side cutout. Upper-thigh hemline 40-42cm.',
    ...
    hosiery: 'HOSIERY: Black thigh-high stockings with sapphire welt. 8-denier T=0.90.',
  },
  {
    name: '155-photonic-microprism-harness',
    scene: 'Pepper’s-ghost scrim shows a faint floating bar logo above the counter.',
    geometry: 'Microdress with low open back and open sides bridged by a thin harness over the midriff. Upper-thigh hemline 40-42cm.',
    ...
    hosiery: 'HOSIERY: Black thigh-high stockings with metallic welt. 8-denier T=0.90.',
  },
  {
    name: '156-thermochromic-iris-cutout',
    scene: 'Water-channel caustics flicker across the bar lip and lower wall.',
    geometry: 'Strapless microdress with large side cutouts and open back to the waist. Upper-thigh hemline 40-42cm.',
    ...
    hosiery: 'HOSIERY: Black thigh-high stockings with burgundy welt. 8-denier T=0.90.',
  },
  {
    name: '157-graphene-lace-suspension',
    scene: 'Brass lattice truss casts geometric shadow bands across the booths.',
    geometry: 'Lace microdress with thin straps, open back to the waist, and high side cutouts; opaque bust and hip panels. Upper-thigh hemline 40-42cm.',
    ...
    hosiery: 'HOSIERY: Black thigh-high stockings with matte welt. 8-denier T=0.90.',
  },
  {
    name: '158-quantum-dot-veil-bridges',
    scene: 'Projection patterns crawl across the vault door and spill onto the floor.',
    geometry: 'Black satin microdress with open sides linked by micro-chain bridges; low open back. Upper-thigh hemline 40-42cm.',
    ...
    hosiery: 'HOSIERY: Black thigh-high stockings with crimson welt. 8-denier T=0.90.',
  },
  {
    name: '159-plasmonic-foil-tension',
    scene: 'Amber liquor wall glows behind the bar, reflecting in black marble.',
    geometry: 'Metallic foil microdress with a narrow underbust cutout and open back to the waist; thin straps. Upper-thigh hemline 40-42cm.',
    ...
    hosiery: 'HOSIERY: Black thigh-high stockings with gold welt. 8-denier T=0.90.',
  },
  {
    name: '160-auxetic-microgrid-expose',
    scene: 'LED mesh bottle wall adds a faint cool fill.',
    geometry: 'Microdress with open back to the waist and an auxetic microgrid midriff cage; high-cut sides. Upper-thigh hemline 40-42cm.',
  ...
  {
    name: '161-electrochromic-lamella-spine',
    scene: 'Blue-violet spill grazes the smoked mirror ceiling.',
    geometry: 'Panelled microdress with low open back; lamella side panels and a narrow spine opening. Upper-thigh hemline 40-42cm.',
    ...
    hosiery: 'HOSIERY: Black thigh-high stockings with cobalt welt. 8-denier T=0.90.',
  },
  {
    name: '162-hydrogel-microlens-sheers',
    scene: 'Candle cylinders produce warm pools with deep shadow falloff.',
    geometry: 'Sheer-panel microdress with open back to the waist and narrow side cutouts; satin bust and hip panels. Upper-thigh hemline 40-42cm.',
    ...
    hosiery: 'HOSIERY: Nude thigh-high stockings with pearl welt. 8-denier T=0.90.',
  },
  {
    name: '163-viscoelastic-crystal-cowl',
    scene: 'Low haze diffuses the projection mapping into soft bokeh.',
    geometry: 'Microdress with dramatic cowl back and open sides linked by fine straps. Upper-thigh hemline 40-42cm.',
    ...
    hosiery: 'HOSIERY: Black thigh-high stockings with prism welt. 8-denier T=0.90.',
  },
  {
    name: '164-phase-change-banded-satin',
    scene: 'Glass floor reveals neon glow below with faint color bleed.',
    geometry: 'Satin microdress with open back to the waist and asymmetric side cutout. Upper-thigh hemline 40-42cm.',
  ...
  {
    name: '165-microled-fiber-backchain',
    scene: 'Smoked mirror ceiling reflects the candle cylinders into a grid.',
    geometry: 'Microdress with low open back and thin straps; open sides linked by a micro-LED fiber chain. Upper-thigh hemline 40-42cm.',
    ...
    hosiery: 'HOSIERY: Black thigh-high stockings with electric-blue welt. 8-denier T=0.90.',
  },
  {
    name: '166-photoelastic-strap-corset',
    scene: 'Projection-mapped vault door adds soft geometric bands on the wall.',
    geometry: 'Corset microdress with open back to the waist; transparent strap harness across the midriff. Upper-thigh hemline 40-42cm.',
    material: 'Satin corset base with transparent photoelastic straps.',
    micro: 'PHOTOELASTIC STRAPS: stress-induced birefringence creates faint rainbow bands at load points; strap edges show crisp specular lines and fine casting flow texture.',
    hosiery: 'HOSIERY: Black thigh-high stockings with graphite welt. 8-denier T=0.90.',
  },
  {
    name: '167-superhydrophobic-slit-silk',
    scene: 'Amber bottle wall glow reflects in the bar’s black marble surface.',
    geometry: 'Silk microdress with diagonal front slit and open back to the waist; thin straps. Upper-thigh hemline 40-42cm.',
    ...
    hosiery: 'HOSIERY: Black thigh-high stockings with satin welt. 8-denier T=0.90.',
  },
  {
    name: '168-metaweave-interference',
    scene: 'Brass lattice truss creates repeating shadow grids across the bar.',
    geometry: 'Microdress with open back to the waist and high side cutouts; narrow underbust aperture. Upper-thigh hemline 40-42cm.',
  ...
  {
    name: '169-tribo-mechano-chain',
    scene: 'LED mesh bottle wall flickers subtly, adding cool fill.',
    geometry: 'Microdress with open sides bridged by chain links; open back to the waist. Upper-thigh hemline 40-42cm.',
  ...
  {
    name: '170-acousto-optic-iris-drape',
    scene: 'Kinetic brass iris overhead throws moving specular highlights across the room.',
    geometry: 'One-shoulder microdress with open back to the waist and a narrow side cutout; soft drape at the hip. Upper-thigh hemline 40-42cm.',
...
console.log(`\n=== V19 APEX - Two-pass, multi-turn refinement ===`);
...
for (let i = s; i < Math.min(e, concepts.length); i++) {
  const conceptNum = i + 151;
  if (existingNums.has(conceptNum)) {
    console.log(`SKIP [${conceptNum}/170] ${concepts[i].name} (already exists)`);
