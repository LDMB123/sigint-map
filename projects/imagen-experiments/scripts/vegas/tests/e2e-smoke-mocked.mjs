...
const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'speakeasy-e2e-mock-'));
...
try {
  ...
  if (result.code !== 0) {
    throw new Error(
      `Mocked smoke run failed with exit ${result.code}\nSTDOUT:\n${result.stdout}\nSTDERR:\n${result.stderr}`
    );
  }
  ...
  const runDirs = (await fs.readdir(outputBase))
    ...
    .sort();
  assert(runDirs.length > 0, 'No run directory created by mocked smoke run');
  ...
  const attemptsCount = Number(totals.primarySuccess || 0) + Number(totals.safeSuccess || 0) + Number(totals.failed || 0);
  assert(Number(totals.prompts) === 1, `Expected 1 prompt in summary, got ${totals.prompts}`);
  assert(attemptsCount === 1, `Expected exactly 1 attempted prompt, got ${attemptsCount}`);
  assert(Number(totals.primarySuccess || 0) >= 1, 'Expected primary success in mocked smoke run');
  ...
  console.log('PASS mocked E2E smoke: runner + quality gate + summary path are healthy.');
