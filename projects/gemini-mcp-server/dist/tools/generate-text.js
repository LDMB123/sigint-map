...
export function registerGenerateTextTool(server, client) {
    server.tool("generate_text", "Generate text using Google Gemini models. Supports various Gemini models for text generation tasks.", {
        ...
        try {
            ...
            const result = await client.generateContent(model, prompt, Object.keys(config).length > 0 ? config : undefined);
