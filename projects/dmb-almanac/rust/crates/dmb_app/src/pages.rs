...
#[cfg(feature = "hydrate")]
fn spawn_local_to_send<T: Default + Send + 'static>(
...
#[cfg(feature = "hydrate")]
fn now_ms_i64() -> i64 {
    let now = js_sys::Date::now().trunc();
    if !now.is_finite() {
        return 0;
    }
    let clamped = now.clamp(i64::MIN as f64, i64::MAX as f64);
    format!("{clamped:.0}").parse::<i64>().unwrap_or(0)
}
