...
fn stable_id_from_string(value: &str) -> i32 {
    ...
    let i32_max_u32 = u32::try_from(i32::MAX).unwrap_or(u32::MAX);
    let bounded = raw % i32_max_u32;
    i32::try_from(bounded).unwrap_or(i32::MAX)
}
    ...
    fn cache_read_error_is_tracked() {
        with_warning_lock(|| {
            ...
            let temp_dir = std::env::temp_dir()
                .join(format!("dmb_scrape_cache_read_{}", rand::random::<u64>()));
            fs::create_dir_all(&temp_dir).unwrap_or_else(|err| panic!("create temp dir: {err}"));
