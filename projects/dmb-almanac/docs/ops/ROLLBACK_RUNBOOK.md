...
4. IndexedDB migration deletes prior prototype data incorrectly.
5. Service worker update loop or failure to activate.

## Server-Side Rollback (Traffic)
1. Pin traffic to a known-good Rust build immediately (git tag/commit or artifact).
2. If available, keep the failing Rust build accessible on a separate origin for debugging.
...
1. In-app recovery:
   ...
   - Use "Cleanup old caches" if available and online (this deletes old CacheStorage entries only).
...
3. Keep rollback window explicit:
   - If the team cannot commit to supporting the pinned Rust build, do not roll back without a plan.
...
2. Reproduce in a clean profile and a profile that has prior offline data.
