    ...
    let callback = Closure::wrap(Box::new(move |list: JsValue, _observer: JsValue| {
        ...
        if let Ok(entries) = js_sys::Reflect::get(&list, &JsValue::from_str("getEntries")) {
            if let Ok(func) = entries.dyn_into::<js_sys::Function>() {
                if let Ok(entries_arr) = func
                    .call0(&list)
                    .and_then(|v| v.dyn_into::<js_sys::Array>())
                {
                    if let Ok(last_entry) = entries_arr
                    ...
                    .and_then(|v| v.dyn_into::<js_sys::Array>())
                {
                    if let Ok(entry) = entries_arr.at(0).dyn_into::<web_sys::PerformanceEntry>() {
                        ...
                        let processing_start =
                            js_sys::Reflect::get(&entry, &JsValue::from_str("processingStart"))
                                .ok()
                                .and_then(|v| v.as_f64())
                                .unwrap_or(0.0);
            ...
            if let Ok(func) = entries.dyn_into::<js_sys::Function>() {
                    ...
                    .and_then(|v| v.dyn_into::<js_sys::Array>())
                {
                    for i in 0..entries_arr.length() {
                        if let Ok(entry) = entries_arr
                            .at(i as i32)
                            .dyn_into::<web_sys::PerformanceEntry>()
                        {
                            // Check hadRecentInput property
                            let had_recent_input =
                                js_sys::Reflect::get(&entry, &JsValue::from_str("hadRecentInput"))
                                    .ok()
                            ...
                            if !had_recent_input {
                                let value =
                                    js_sys::Reflect::get(&entry, &JsValue::from_str("value"))
                                        .ok()
                                        .and_then(|v| v.as_f64())
                                        .unwrap_or(0.0);
                                cls_score += value;
