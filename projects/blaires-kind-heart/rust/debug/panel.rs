...
impl DebugPanel {
    ...
    fn create_header(&self) -> Element {
        ...
        // Title
        let title = dom::document().create_element("h3").unwrap();
        title.set_text_content(Some("🔧 Debug Panel"));
        header.append_child(&title).ok();

        // Close button
        let close_btn = dom::document().create_element("button").unwrap();
        ...
        close_btn.add_event_listener_with_callback("click", closure.as_ref().unchecked_ref()).ok();
        closure.forget();
    ...
    fn create_tab_bar(&self) -> Element {
        let tabs = dom::document().create_element("div").unwrap();
    ...
    fn create_tab_button(&self, tab_type: DebugTab, name: &str, emoji: &str) -> Element {
        let btn = dom::document().create_element("button").unwrap();
    ...
    fn create_content_area(&self) -> Element {
        let content = dom::document().create_element("div").unwrap();
