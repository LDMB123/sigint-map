---
name: sveltekit-a11y-keyboard-test
description: Accessibility keyboard/focus test harness for UI components
...
Systematically test keyboard navigation and focus management in SvelteKit applications.

## When to Use
- After migrating to native HTML elements
...
4. Verify focus indicator not clipped by overflow
```
...
7. Click outside closes (if light-dismiss)
```
...
Verify role and state announcements
```

### 9. Automated Testing (Playwright)

```typescript
// Playwright test example for SvelteKit
...
### 10. Svelte Component Testing

```typescript
// Vitest component test with keyboard interaction
...
- [ ] Light-dismiss works (if applicable)
...
| Artifact | Purpose |
|----------|---------|
| Checklist (completed) | Accessibility evidence |
